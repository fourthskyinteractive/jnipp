...
{
	jclass cls = getEnv()->FindClass( name );
	if ( cls == NULL )
...
{
	jmethodID mid = getEnv()->GetMethodID( clazz, name, sig );
	if ( mid == NULL )
...
{
	jfieldID fid = getEnv()->GetFieldID( clazz, name, sig );
	if ( fid == NULL )
...
{
	jmethodID mid = getEnv()->GetStaticMethodID( clazz, name, sig );
	if ( mid == NULL )
		throw MethodNotFoundException( name );
	return mid;
}
...
{
	jfieldID fid = getEnv()->GetStaticFieldID( clazz, name, sig );
	if ( fid == NULL )
		throw FieldNotFoundException( name );
	return fid;
}
...
{
	return getEnv()->RegisterNatives( clazz, methods, nMethods );
}
