...
//		if ( root.getPackageName().equals( "" ) == true )
			fullFileName = root.getCPPClassName() + "Mapping.cpp";
//		else
//			fullFileName = root.getPackageName().replace( '.', File.separatorChar ) + File.separatorChar + root.getCPPClassName() + "Mapping.cpp";
	...
	public void generateInitMethod(ClassNode root, FormattedFileWriter writer)
		throws java.io.IOException
	{
   	...
   	writer.outputLine( "return reinterpret_cast<" + root.getCPPClassName() + "Peer*>( peerPtr );" );
		...
		while ( it.hasNext() == true )
		{
  	      ...
  	      if ( node.getReturnType().getPlainJNITypeName().equals( "void" ) == false )
				writer.outputLine( node.getReturnType().getPlainJNITypeName() + " rVal;\n" );
         writer.outputLine( "try" );
         writer.outputLine( "{" );
         writer.incTabLevel();
  	      if ( node.getReturnType().getPlainJNITypeName().equals( "void" ) == false )
