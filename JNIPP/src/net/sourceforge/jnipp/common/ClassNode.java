...
public class ClassNode
{
	...
	public void init()
		...
		else
		{
			...
			{
				for ( arrayDims = 0;
						theClass.isArray() == true;
						++arrayDims, theClass = theClass.getComponentType() )
					;
		...
		while ( ctorIter.hasNext() == true )
		{
			...
			while ( paramIter.hasNext() == true )
			{
				...
				if ( current.isPrimitive() == false && dependencyList.contains( current ) == false )
				{
					...
					if ( current.isArray() == true && current.getComponentType() != null && current.getComponentType().needsProxy() == true )
						dependencyList.add( current.getComponentType() );
				}
			}
		}
	...
	public String getFullyQualifiedClassName()
	{
		...
		{
			if ( componentType.packageName.equals( "" ) == true )
				return getClassName();
	...
	public String getJNITypeName(boolean usePartialSpec)
	{
		...
		if ( isArray() == true && componentType.needsProxy() == true )
			if ( usePartialSpec == true )
				return "::net::sourceforge::jnipp::ProxyArray< " + componentType.getJNITypeName( usePartialSpec ) + ", " + arrayDims + " >";
			else
